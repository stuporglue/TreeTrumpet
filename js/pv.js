(function($){
    ...
    function EventGenerator(eventMethod) {
        var _listeners = [];
    ...
    * @return
    */
    function StandardChartRenderer(options) {
        ...
        var _personStyles = { 
            ...
            'withspouse' : function(p) {
                var s = p.spouse ? p.spouse : p.sp ? pedigreeViewer.findPerson(p.sp) : null;
            ...
            'married to' : function(p) {
                var s = p.sp ? pedigreeViewer.findPerson(p.sp) : null;
                if (s) return 'Spouse: ' + _formatPerson(s, FULLNAME);
                return '';
            },
            'father' : function(p) {
                if (p.f != null) {
                    return 'Father: ' + _formatPerson(pedigreeViewer.findPerson(p.f), FULLNAME);
                }
                return '';
            },
            'mother' : function(p) {
                if (p.m != null) {
                    return 'Mother: ' + _formatPerson(pedigreeViewer.findPerson(p.m), FULLNAME);
                }
                return '';
            },
            'children' : function(p) {
                var o = pedigreeViewer.findOffspring(p.id);
                if (o && o.length > 0) {
                    var t = 'Children :<br/>';
    ...
    */
    function OutlineChartRenderer(params) {
    ...
    function ProxyDragger(container, element, options) {
    ...
    function PedigreeViewerUI(){
        ...
        var _params = {
            ...
            classes : {
                couple:'pvCouple',
                individual:'pvIndividual',
                hover:'pvHover',
                selected:'pvSelected',
                male:'pvMale',
                female:'pvMemale',
                blank:'pvBlank-couple'
            },		
        ...
        var pedigreeViewer = null, focus = null;
                    ...
                    var _drawOne = function(person) {				
                        if (person) {
                            if(!person.father && !person.isBlank) {
                                pedigreeViewer.addBlankFather(person);
                            }
                            if(!person.mother && !person.isBlank) {
                                pedigreeViewer.addBlankMother(person);
                            }
        ...
        */
        var _redraw = function(params) {		
            ...
            var focus = pedigreeViewer.getFocus();
        ...
        /**
        * notification from PedigreeViewer object that the chart was updated.  data is the newly updated
        ...
        */
        ...
        /**
        * PedigreeViewerUI public API and UI functionality.
        */

        this.initialize = function(_pedigreeViewer, params) { 
            pedigreeViewer = _pedigreeViewer;
            pedigreeViewer.addListener({chartUpdated:_update});
            focus = pedigreeViewer.getFocus();
            ...
            if (!isStatic) {
                ...
                var hoverCache = new SimpleCache(function(item) {
                    ...
                    var a = pedigreeViewer.findLineageList(item.attr(REL));
                    var ids = "";
            ...
            if (params.preview) {			
                ...
                previewUI = new PedigreeViewerUI();						
                previewUI.initialize(pedigreeViewer, params.preview);
                var previewChart = previewUI.getChartDiv();						
                var chartPreviewContainer = previewChart.parent();
    ...
    function SimpleCache(idGenerator, valueGenerator) {
        var c = {};
    ...
    */
    function PedigreeViewer(target,dataUrl,options) {
        ...
        options = options || {};
        var settings = $.extend({dataUrl:dataUrl},_defaults,options);

        this.load = function(options, requestParameters) {
            ui = new PedigreeViewerUI();
            requestParameters = requestParameters || {};
            var finalReqParams = $.extend({ a: options.a, d: options.d, nfsid: options.nfsid }, requestParameters);
            $.getJSON(options.dataUrl, finalReqParams, function(d) {
                pedigreeViewer = PedigreeViewer.getInstance(d['data']);				
                ui.initialize(pedigreeViewer, options);
        ...
        // pan
        $(".pvChartControls .pan").bind("click", function() { self.pan($(this).attr("rel")); });

        // orientation
        $(".pvChkOrientation").click(function() {
            self.setOrientation($(this).attr("rel"));
        });			

        var pedigreeViewer = null, ui = null;
        var panFuncs = null; 
        ...
        */
        ...
        var _pedigreeViewer = function(data) {
            _people = data.people;
            _focus = data.focus;
            _focusPerson = _findPerson(_focus);
            _initialize();
        ...
        var PedigreeViewer = {
            getInstance : function(data) {
                return new _pedigreeViewer(data);
            }			
        };	
